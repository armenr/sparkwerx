...
self_source=$repo_dir/scripts/switch-root-canary-generation-pilot.sh
...
timer_armed=false
generation_two_root_created=false
...
on_exit() {
  ...
  if ((status != 0)); then
    if [[ "$timer_armed" == true ]]; then
      printf '%s\n' \
        "ROLLBACK ARMED: keep both $pilot_root and $generation_two_root; $rollback_unit.timer will run the exact generation-one rollback." \
        >&2
    elif [[ "$switch_started" == true ]]; then
      printf '%s\n' \
        "WARNING: switching started without a known armed timer; from the console run: $transaction rollback-switch $generation_one $generation_two" \
        >&2
    elif [[ "$generation_two_root_created" == true ]]; then
      printf '%s\n' \
        "NO SWITCH STARTED: exact generation-two retention was created and left at $generation_two_root for explicit verification/cleanup." \
        >&2
    fi
  fi
}
...
assert_snapshot_fresh() {
  ...
  ((snapshot_age >= 0 && snapshot_age <= 1800)) ||
    die "snapshot is not in the current 30-minute generation-switch window (age=${snapshot_age}s)"
}
...
assert_exact_manager_state() {
  ...
  ' "$state_path" >/dev/null ||
    die "System Manager state exceeds or differs from the exact five-path/three-service boundary"
}
...
assert_snapshot_registration_inventory() {
  ...
  [[ "${#records[@]}" -eq 8 ]] ||
    die "snapshot registration inventory must contain exactly eight records"
  [[ "${records[0]}" == "EXACT_DIRECTORY|$profile_dir" ]] ||
    die "snapshot profile-directory record is not exact"
  [[ "${records[1]}" == "EXACT_SYMLINK|$profile_path|system-manager-1-link|$generation_one" ]] ||
    die "snapshot selected-profile record is not exact generation one"
  ...
  [[ "${records[3]}" == "ABSENT|$generation_two_path" ]] ||
    die "snapshot did not record generation two absent"
  [[ "${records[4]}" == "EXACT_SYMLINK|$gcroot_path|$generation_one|$generation_one" ]] ||
    die "snapshot upstream-root record is not exact generation one"
  [[ "${records[5]}" == "EXACT_SYMLINK|$pilot_root|$generation_one|$generation_one" ]] ||
    die "snapshot generation-one pilot-root record is not exact"
  [[ "${records[6]}" == "ABSENT|$generation_two_root" ]] ||
    die "snapshot did not record generation-two pilot root absent"
  [[ "${records[7]}" == "ACTIVE_REGISTERED_RETAINED|$state_path" ]] ||
    die "snapshot did not record exact registered/live generation one"
}
...
assert_snapshot() {
  ...
  for exact_record in \
    schema=1 \
    purpose=system-manager-generation-switch \
    host=sparkle-01 \
  ...
  [[ -n "$snapshot_helper_sha" &&
    "$snapshot_helper_sha" == "$(sha256sum "$repo_dir/scripts/snapshot-root-generation-switch.sh" | awk '{print $1}')" ]] ||
  ...
  transaction=$snapshot/root-generation-switch-transaction.sh
  ...
  [[ "$(sha256sum "$transaction" | awk '{print $1}')" == "$transaction_sha256" &&
    "$(sha256sum "$repo_dir/scripts/root-generation-switch-transaction.sh" | awk '{print $1}')" == "$transaction_sha256" ]] ||
  ...
  [[ -z "$current_status" ]] ||
    die "repository must remain clean for the generation switch"
...
assert_current_flake_evidence() {
  local current_generation_one current_generation_two current_test_drv current_test_output
  ...
  snapshot_sha="$(sha256sum "$repo_dir/scripts/snapshot-root-generation-switch.sh" | awk '{print $1}')"
  ...
  jq -e \
    ...
    --arg parser_test_sha "$parser_test_sha" '
      ...
      .registration.guardedGenerationSwitch.exactCandidates.generationOne ==
        $generation_one and
      .registration.guardedGenerationSwitch.exactCandidates.generationTwo ==
        $generation_two and
...
assert_generation_one_prestate() {
  ...
  if path_exists "$generation_two_root"; then
    die "generation-two pilot root already exists before this transaction"
  fi
  if path_exists "$generation_two_path"; then
    die "generation-two profile link already exists before this transaction"
  fi
  if path_exists "$boot_link"; then
    die "boot linkage is forbidden during the generation-switch pilot"
  fi
  ...
  assert_system_health
  assert_tailscale_health
  pass preflight "exact registered/live generation one is unchanged; generation two and boot linkage remain absent"
}

assert_generation_one_with_second_retention() {
  "$transaction" verify-before "$generation_one" "$generation_two" >/dev/null ||
    die "transaction rejected generation one after exact generation-two retention"
  cmp -s "$state_path" "$snapshot/manager-state.before.json" ||
    die "candidate retention changed live manager state"
  assert_managed_links_match_snapshot
  sha256sum -c "$snapshot/protected-files.before.sha256" >/dev/null ||
    die "candidate retention changed a protected account/Nix file"
  pass retention "generation two is directly retained; generation one remains selected, extra-rooted, and live"
}

assert_generation_two_state() {
  "$transaction" verify-after "$generation_one" "$generation_two" >/dev/null ||
    die "transaction verifier rejected registered/live generation two"
  assert_exact_manager_state
  grep -Fx 'registration-test-generation=2' /etc/dgx-setup/canary >/dev/null ||
    die "live canary lacks the exact generation-two marker"
  ...
  [[ ! -e "$boot_link" && ! -L "$boot_link" ]] ||
    die "boot linkage appeared during the generation switch"
  sha256sum -c "$snapshot/protected-files.before.sha256" >/dev/null ||
    die "generation switch changed a protected account/Nix file"
  assert_protected_units
  assert_system_health
  assert_tailscale_health
  pass post_switch "exact generation two is selected, extra-rooted, and live; generation one and both pilot roots remain; no boot link exists"
}
...
[[ "$(readlink -f -- "${BASH_SOURCE[0]}")" == "$self_source" ]] ||
  die "run the exact repository live-switch wrapper, not a copy or symlink"
...
[[ -t 0 && -t 1 ]] ||
  die "run this guarded generation switch from an interactive terminal"

printf '# DGX System Manager generation-switch pilot\n'
printf '# timestamp_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
info generation_one "$generation_one"
info generation_two "$generation_two"
info retention "$pilot_root;$generation_two_root"
info rollback "$rollback_unit.timer;10 minutes;exact registered/live generation-one rollback"
info unchanged "boot linkage;root-manager ownership;all factory/access services"
...
ln -s -- "$generation_two" "$generation_two_root" ||
  die "could not create exact generation-two pilot root"
generation_two_root_created=true
[[ -L "$generation_two_root" && "$(readlink -- "$generation_two_root")" == "$generation_two" ]] ||
  die "generation-two pilot root target verification failed"
assert_generation_one_with_second_retention

if ! systemd-run \
  --unit="$rollback_unit" \
  --description="Timed rollback for DGX System Manager generation switch" \
  --collect \
  --service-type=exec \
  --on-active=10m \
  --timer-property=AccuracySec=1s \
  "$transaction" rollback-switch "$generation_one" "$generation_two"; then
  if [[ "$(systemctl show "$rollback_unit.timer" -p ActiveState --value 2>/dev/null || true)" == active ]]; then
    timer_armed=true
  fi
  die "could not establish the exact generation-one rollback timer"
fi
timer_armed=true
...
for expected_arg in "$transaction" rollback-switch "$generation_one" "$generation_two"; do
  grep -F -- "$expected_arg" <<<"$exec_start" >/dev/null ||
    die "rollback service does not contain exact argument: $expected_arg"
done
pass rollback "timer is active/waiting and bound to exact registered/live generation-one rollback"
...
switch_started=true
"$transaction" apply-switch "$generation_one" "$generation_two"
assert_generation_two_state

printf '\n%s\n' \
  'Automatic postflight passed. Verify the physical keyboard/display/local terminal still works.' \
  'Keep this terminal alive too. Within five minutes, type exactly KEEP GENERATION TWO.' \
  'Any other input, Ctrl-C, disconnect, or timeout leaves the ten-minute generation-one rollback armed.' \
  "Immediate manual rollback is: $transaction rollback-switch $generation_one $generation_two" \
  'Rollback restores registered/live generation one and deliberately keeps both exact pilot roots.'
printf '> '

console_reply=
if ! read -r -t 300 console_reply; then
  die "local-console confirmation timed out; leaving generation-one rollback armed"
fi
[[ "$console_reply" == "KEEP GENERATION TWO" ]] ||
  die "confirmation did not match; leaving generation-one rollback armed"
...
"$transaction" verify-after "$generation_one" "$generation_two" >/dev/null ||
  die "generation-two state changed while disarming rollback"
[[ -L "$pilot_root" && "$(readlink -- "$pilot_root")" == "$generation_one" ]] ||
...
pass generation_switch "generation two retained after repeated postflight and independent local-console confirmation"
printf '%s\n' \
  "KEEP: $pilot_root and $generation_two_root remain required for exact rollback." \
  "REGISTERED/LIVE: $profile_path and $gcroot_path point to exact generation two." \
  'NOT DONE: no boot linkage, reboot, broader root role, desktop switch, Tailscale migration, or pilot-root cleanup occurred.'
