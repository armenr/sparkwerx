...
| Root desktop controller | Headless/factory-GNOME targets | DISPOSABLE LIFECYCLE PASSED; LIVE HOST OPEN | Two thin System Manager candidates add no packages or daemon: headless is 5,536 bytes and GNOME is 5,504 bytes above exact generation four. Their active-service map remains identical to generation four; immutable mode targets preserve `system-manager.target`/Tailscale, own no GDM unit, and restore the factory default when removed. The exact 10-subtest/three-reboot disposable lifecycle passed with clean live-host postflight | Design and pass a timed guarded live-switch transaction; do not activate or isolate from the raw outputs |
