...
| Fleet operator | `scripts/dgx-setup` | COMPLETE FOR RETAINED DECLARED STATE | On exact `sparkle-01`, the complete regression at commit `9883d6c` returns `PLAN_STATUS=READY` and `APPLY_STATUS=COMPLETE`: Nix, Home, Nix-managed Tailscale, and System Manager generation five/headless all converge as no-ops while root state, services, and mutable data remain exact. See the [current integration authority](../root/desktop/validation/2026-09-05-confirmed-headless-integration.md) | A pristine host can bootstrap Nix, but its proven one-time Tailscale and desktop transitions are not yet automatically composed; keep using their guarded operators until that orchestration is implemented and tested |
