...
cleanup passed. Current state is
`ACTIVE_REGISTERED_GENERATION_TWO_TRIPLE_RETAINED`: generations one/two remain
registered, generation two is selected/upstream-rooted/live, generation three
remains directly rooted, and the boot/recovery edges are absent. Updated
hash-pinned helpers accept Nix's normal socket-idle postboot state and expose a
short operator route but no reboot action. Guarded no-reboot restoration of
generation three is next.
...
| Root integration | System Manager | SELECTED; ALL SIX DISPOSABLE TESTS PASSED; FIRST REAL REBOOT/AUTOMATIC ROLLBACK VERIFIED; GENERATION TWO LIVE/REGISTERED; THREE DIRECT ROOTS RETAINED; RECOVERY CLEAN/UNARMED | Exact 1.1.0 pin on matching `release-26.05`; inert ARM64 closure is 109 paths / 230.0 MiB. Its private wrapper is verified Nix 2.35.2, and all six exact disposable derivations remain policy-pinned. The 13-subtest recovery design then survived the separately authorized first real reboot: its deadline fired, exact no-boot generation two was restored, verification passed, and the recovery surface was cleaned. `system-manager -> system-manager-2-link -> pmrq…` and `system-manager-current -> pmrq…`; generation-three's numbered link/boot edge are absent, but all three direct pilot roots retain their exact candidates. Current classifier is `ACTIVE_REGISTERED_GENERATION_TWO_TRIPLE_RETAINED`. Updated postboot checks accept a healthy socket-activated Nix runtime, and the short operator/restoration helpers contain no reboot action | Preserve all three direct roots and exact generation-two state until guarded restoration. Do not reuse spent snapshots/helpers, remove a generation/root, arm recovery, reboot, or add a real managed service without its current reviewed gate. Next step is the timed, local-console-confirmed, no-reboot generation-three restoration |
