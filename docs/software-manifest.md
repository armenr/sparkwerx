...
| Bootstrap | Official Nix installer | CURRENT PIN; READ-ONLY PLAN IMPLEMENTED; EXECUTION OPEN | `bootstrap/nix/source.json` pins official `NixOS/nix-installer` 2.35.1 for ARM64 Linux at 32,536,352 bytes and SHA-256 `7e6e2f75...e0e0e649`. The installed `/nix/nix-installer` matches it byte-for-byte. `scripts/update-nix-installer.sh --check` independently validates the latest GitHub release metadata and downloaded binary; `scripts/dgx-setup plan` classifies exact adoption versus clean install versus foreign/partial hold without executing it | Design and test the idempotent install/adopt transaction plus complete rollback/uninstall before using this pin on a pristine machine; keep runtime updates separate |
