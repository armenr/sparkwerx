...
| Root desktop controller | Headless/factory-GNOME targets | CANDIDATES BUILT; LIVE HOST OPEN | Two thin System Manager candidates add no packages or daemon: headless is 5,536 bytes and GNOME is 5,504 bytes above exact generation four. Their active-service map remains identical to generation four; immutable mode targets preserve `system-manager.target`/Tailscale, own no GDM unit, and restore the factory default when removed. Static policy passed; [disposable lifecycle is the next gate](2026-09-05-desktop-controller-candidates.md) | Run the root-assisted disposable lifecycle, then design a timed guarded live switch; do not activate or isolate from the raw outputs |
