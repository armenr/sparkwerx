...
| Fleet operator | `scripts/dgx-setup` | COMPLETE FOR RETAINED DECLARED STATE; FRESH-HOST CANDIDATE IMPLEMENTED/FINAL GATE PENDING | On exact `sparkle-01`, the complete regression at commit `9883d6c` returns `PLAN_STATUS=READY` and `APPLY_STATUS=COMPLETE` as a no-op. The new resumable `converge` lane composes exact Nix bootstrap, generic per-declaration Home, optional Nix Tailscale, factory generation one, a separately initiated reboot, and headless generation two with persistent rollback. It never reboots and recognizes the historical pilot without changing it. See [fresh-host convergence](fresh-host-convergence.md) | Do not use the fresh lane on another DGX until `scripts/test-fresh-host-convergence-integration.sh` passes from a clean commit and exact evidence is recorded; later root-generation updates and enabled workloads remain separate |
