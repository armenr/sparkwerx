...
| Root desktop controller | Headless/factory-GNOME targets | FIRST LIVE ATTEMPT SAFELY ROLLED BACK; DASHBOARD-AWARE REVALIDATION REQUIRED | The thin candidates still add no desktop package or daemon. Attempt one reached exact headless generation five with Tailscale intact, then failed closed because the factory `dgx-dashboard.service` GUI was correctly stopped but incorrectly classified as mode-independent. Persistent rollback restored exact generation four/GNOME, and exact cleanup passed. The corrected contract keeps the GUI Dashboard inactive in headless and active in GNOME while `dgx-dashboard-admin.service`, Docker, NVIDIA persistence, and Tailscale remain active. All three disposable fixtures now model the real Dashboard `default.target` edge; their previous results are superseded until rerun | Run the one-command Dashboard-aware regression, record exact current evidence, then use only `scripts/dgx-desktop` for a separately authorized retry; never activate a raw output or isolate directly. See the [host attempt](../root/desktop/validation/2026-09-05-host-attempt-1.md) |
