...
short operator route but no reboot action. The first no-reboot restoration
reached healthy generation three, but a typo in the old retention phrase left
the exact timer armed; it safely restored this same generation-two state. The
corrected retry uses one pre-mutation Enter and retains automatically only after
two successful postflights.
...
| Root integration | System Manager | SELECTED; ALL SIX DISPOSABLE TESTS PASSED; FIRST REAL REBOOT/AUTOMATIC ROLLBACK VERIFIED; RESTORATION ATTEMPT ONE SAFELY ROLLED BACK; GENERATION TWO LIVE/REGISTERED; THREE DIRECT ROOTS RETAINED; RECOVERY CLEAN/UNARMED | Exact 1.1.0 pin on matching `release-26.05`; inert ARM64 closure is 109 paths / 230.0 MiB. Its private wrapper is verified Nix 2.35.2, and all six exact disposable derivations remain policy-pinned. The 13-subtest recovery design survived the separately authorized first real reboot. Restoration attempt one then reached healthy generation three but safely timed back after the old phrase was mistyped. `system-manager -> system-manager-2-link -> pmrq…` and `system-manager-current -> pmrq…`; generation-three's numbered link/boot edge are absent, but all three direct pilot roots retain their exact candidates. Current classifier is `ACTIVE_REGISTERED_GENERATION_TWO_TRIPLE_RETAINED`. The corrected helper uses one Enter, two automatic postflights, automatic retention, and exact in-flight resume; it still contains no reboot action | Preserve all three direct roots and exact generation-two state until the retry-safe restoration. Do not reuse spent snapshots/helpers, remove a generation/root, arm recovery, reboot, or add a real managed service without its current reviewed gate |
