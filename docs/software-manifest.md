...
| Root desktop controller | Headless/factory-GNOME targets | CANDIDATE/TRANSACTION PASSED; GUARDED-OPERATOR LIFECYCLE OPEN | Two thin System Manager candidates add no packages or daemon: headless is 5,536 bytes and GNOME is 5,504 bytes above exact generation four. Their active-service map remains identical to generation four; immutable mode targets preserve `system-manager.target`/Tailscale, own no GDM unit, and restore the factory default when removed. The 10-subtest/three-reboot mode lifecycle and exact 12-subtest transaction primitive passed. A short operator now snapshots the host and arms an isolate-resistant, persistent ten-minute factory rollback before a detached switch | Pass and record `test-desktop-switch-lifecycle.sh`; do not activate a raw output or run the live operator until that exact evidence is current |
