...
whole-record systemd parser are now implemented and policy-pinned, with the
parser regression exposed as a Nix check. They have not been run on the host;
no host switch or generation-two retention is authorized.
...
| Root integration | System Manager | SELECTED; ALL FOUR DISPOSABLE TESTS PASSED; HOST CANARY ACTIVE; GENERATION ONE REGISTERED/RETAINED; LIVE-SWITCH PILOT DESIGNED/UNRUN; NO BOOT LINK | Exact 1.1.0 pin on matching `release-26.05`; inert ARM64 closure is 109 paths / 230.0 MiB. Its exact-version `skip-empty-tmpfiles` patch prevents global factory-rule processing when the managed set is empty. It adds no global package, port, boot link, user, wrapper, PATH hook, or NVIDIA/Tailscale/desktop ownership. Its private wrapper is verified Nix 2.35.2; Nix 2.34.8 and real `userborn` are closure-rejected. The patched activation/deactivation test, two-generation registration lifecycle test at `m4zm42h…`, guarded first-generation failure-injection test at `lxnykcyv…`, and guarded generation-switch failure-injection test at `0llhzgy…` all passed. The original five-path/three-service canary remains the unchanged live activation. Guarded registration attempt 3 retained exactly `system-manager -> system-manager-1-link -> alrc…` plus direct `system-manager-current -> alrc…`; the pilot root also remains exact. Current classifier result is `ACTIVE_REGISTERED_RETAINED`; the generation-two pilot root is absent. Both live postflights, independent console confirmation, protected process continuity, system/GPU health, and sanitized Tailscale SSH passed; the registration rollback was disarmed before its service ran. The later disposable generation-switch postflight was also clean and made no host change. Its separate fresh-snapshot/timed-rollback/local-console pilot is implemented and pinned but unrun | Preserve both profile links and both exact generation-one roots. Do not rerun inactive activation or first-registration helpers, create the generation-two host root, run the live wrapper, reboot, add boot linkage, switch generations, remove registration, retire either pilot root, or add a real managed service without a fresh snapshot and separately reviewed authorization |
