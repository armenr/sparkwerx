...
| Root desktop controller | Headless/factory-GNOME targets | ALL DISPOSABLE LIFECYCLES PASSED; LIVE SWITCH AWAITING | Two thin System Manager candidates add no packages or daemon: headless is 5,536 bytes and GNOME is 5,504 bytes above exact generation four. Their active-service map remains identical to generation four; immutable mode targets preserve `system-manager.target`/Tailscale, own no GDM unit, and restore the factory default when removed. The 10-subtest/three-reboot mode lifecycle, exact 12-subtest transaction primitive, and seven-subtest guarded-operator lifecycle passed. The short operator snapshots the host and arms an isolate-resistant, persistent ten-minute factory rollback before a detached switch | Use only `scripts/dgx-desktop` for a separately authorized live pilot; never activate a raw output, isolate a target directly, or infer reboot authority |
